...
var l = '8';
// 請文字解釋為什麼
console.log(k * l > 21); // true 因為遇到 * 會自動將變數l轉型為 number , 3 * 8 = 24 
...
if (BobPrice>=giftPriceRule || BobIsVip == true) {
  console.log("客戶您好，您有符合贈品資格");
    if (giftNum > 0) { //先判斷是否還有贈品
     giftNum--; // 贈品數量減 1
     console.log("贈品已成功發放！");
     }else{
     console.log("很抱歉，贈品已經發完了！");}
...
if ((playerA === '剪刀' && playerB === '剪刀')||(playerA === '石頭' && playerB === '石頭')
||(playerA === '布' && playerB === '布')) { //三種平手情境
  console.log('平手');
}else if(playerA === '剪刀'){
  if(playerB ==='石頭'){
    console.log('B獲勝');
...
const gymCoach = {
  ...
  backgroud: "王教練擁有 5 年教學經驗，專精於提升學員的肌力與減脂，適合希望快速達成體能目標的學員",
  isOpenfornew :true,
  },
  coachLee:{
    skill:["瑜伽","體態雕塑"],
    ...
    backgroud: "李教練是一位瑜伽大師，擁有 10 年教學經驗，擅長幫助學員雕塑完美體態，適合希望改善姿態與柔軟度的學員。",
    isOpenfornew :false,
  }
...
calorieBurn+= (activities.cycling.caloriesPerMinute*10+
               activities.runningMachine.caloriesPerMinute*30+
...
exerciseRecords.forEach(function(item,index,array){
  if(array[index].duration>=30 && array[index].heartRate>=130){
    totalDuration += array[index].duration
    validDays += 1;
  }
});
